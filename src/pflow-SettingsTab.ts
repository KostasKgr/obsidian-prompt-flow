...
export class PromptFlowSettingsTab extends PluginSettingTab {
    ...
    // Obsidian 1.13.0+ skips display() when getSettingDefinitions() returns items.
    // This imperative fallback renders for older Obsidian versions.
    display(): void {
        ...
        new Setting(containerEl)
            ...
            .addTextArea((text) => {
                text.setPlaceholder("^reflect on\ntodo:\n\\[template\\]")
        ...
        new Setting(containerEl)
            ...
            .addToggle((toggle) =>
                toggle
                    .setValue(this.plugin.settings.showLlmRequests ?? false)
                    .onChange(async (value) => {
                        this.plugin.settings.showLlmRequests = value;
                        await this.plugin.saveSettings();
                    }),
            );
        new Setting(containerEl)
